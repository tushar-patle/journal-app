...
import com.tushar.journal.service.JournalEntryMethodsService;
...
public class JournalEntryController {
	...
	@PutMapping("/id/{id}")
	public ResponseEntity<JournalEntry> updateJournalEntry(@PathVariable ObjectId id, @RequestBody JournalEntry updateJournalEntry) {
		String username = SecurityContextHolder.getContext().getAuthentication().getName();
		User user = userMethodsService.getAllUserEntries(username);
		List<JournalEntry> collectId = user.getJournalEntry().stream().filter(x->x.equals(id)).collect(Collectors.toList());
		
		if(!collectId.isEmpty()) {
			JournalEntry oldJournalEntry = journalEntryMethodsService.getAllJournalEntries(id).orElse(null);
	...
	// to delete all the journal entries from authorized and authenticated user
	@DeleteMapping
